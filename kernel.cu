...
int main(int argc, const char** argv)
{
	...
	for (int i = 0; i < imgSize; ++i) {
		...
		if ( abs(temp) > 20 ) ++count;
	}
	//GPU Error shouldn't be more than 1%
	if (count > imgSize / 100) testGPU = false;

	if (testGPU) cout << "CUDA PASSED! DIFFERENCE COUNT: " << count << endl;
	else cout << "CUDA FAILED! DIFFERENCE COUNT: " << count << endl;

	// vector -> Mat
	Mat outputGPU(imgHeight, imgWidth, CV_8U);
	memcpy(outputGPU.data, imgGPU, imgSize * sizeof(unsigned char));

	imshow("CUDA处理后图片", outputGPU);
